...
const BATCH_END = new Date('2026-05-31T00:00:00')
...
function useLiveCountdown() {
  ...
  function compute() {
    ...
    const toEnd = Math.max(0, Math.floor((BATCH_END - now) / 1000))
    ...
    const daysLeft = Math.floor(toEnd / 86400)
    return { days, hours, mins, secs, daysLeft }
  }
...
export default function DaysCounter() {
  ...
  const { days, hours, mins, secs, daysLeft } = useLiveCountdown()
  ...
  return (
    ...
    >
      ...
      <p className="font-display text-xl italic text-text-primary/50 mb-8">
      ...
      <p className="font-body text-xs text-muted/50 leading-relaxed">
        ...
        {daysLeft > 0
          ? <> · <span className="text-accent-yellow/40">{daysLeft} days</span> left in this chapter</>
